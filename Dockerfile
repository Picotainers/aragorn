...
# Compatibility-first template for aragorn.
...
RUN set -eux; \
    BIN=""; \
    if [ -x "/opt/conda/bin/aragorn" ]; then BIN="/opt/conda/bin/aragorn"; fi; \
    if [ -z "$BIN" ]; then CAND="/opt/conda/bin/$(echo aragorn | tr '-' '_')"; [ -x "$CAND" ] && BIN="$CAND" || true; fi; \
    if [ -z "$BIN" ]; then BIN="$(find /opt/conda/bin -maxdepth 1 -type f -perm -111 -name 'aragorn*' | head -n1 || true)"; fi; \
    test -n "$BIN"; \
...
RUN set -eux; \
    ...
    printf '#!/usr/bin/env bash\nexec "%s" "$@"\n' "$BIN" > /usr/local/bin/aragorn
RUN chmod +x /usr/local/bin/aragorn && rm -f /tmp/tool-entry-path
WORKDIR /data
